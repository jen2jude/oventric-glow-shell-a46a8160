...
export const Route = createFileRoute("/admin/features")({
  head: () => ({
    meta: [{ title: "Features · Admin" }, { name: "robots", content: "noindex, nofollow" }],
  }),
      ...
      {!rows ? (
        ...
        <div className="grid gap-2">
          {rows.map((f) => (
            <div
              key={f.id}
              className="bg-[#141418] border border-white/10 rounded-xl p-4 flex items-center gap-3"
            >
              ...
              <button
                onClick={async () => {
                  setBusy(f.id);
                  await setFn({ data: { id: f.id, enabled: !f.enabled } });
                  refresh();
                  setBusy(null);
                }}
                disabled={busy === f.id}
              ...
              >
                <span
                  className={`absolute top-0.5 w-5 h-5 rounded-full bg-white transition-all ${f.enabled ? "left-6" : "left-0.5"}`}
                />
              </button>
            </div>
          ))}
