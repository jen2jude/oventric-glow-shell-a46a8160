...
type Settings = { base_currency: string; live_fx_enabled: boolean; fx_rates: Record<string, number> };
...
function SettingsPage() {
  ...
  useEffect(() => {
    getFn().then((r) => setS(r as Settings ?? { base_currency: "USD", live_fx_enabled: false, fx_rates: { USD: 1, NGN: 1500, GHS: 14 } }));
  }, [getFn]);
  ...
  const save = async () => {
    ...
    try {
      await upFn({ data: { base_currency: s.base_currency, live_fx_enabled: s.live_fx_enabled, fx_rates: s.fx_rates } });
      setMsg("Saved.");
    ...
    <div className="p-6 max-w-2xl mx-auto">
      ...
      <div className="bg-[#141418] border border-white/10 rounded-xl p-5 grid gap-4">
        <label className="block">
          <div className="text-[10px] uppercase tracking-wider text-slate-500 font-bold mb-1">Base currency</div>
          <select value={s.base_currency} onChange={(e) => setS({ ...s, base_currency: e.target.value })} className="w-full bg-[#0b0b0d] border border-white/10 rounded-lg px-3 py-2 text-sm text-white">
            <option value="USD">USD</option><option value="NGN">NGN</option><option value="GHS">GHS</option>
          </select>
        </label>

        <label className="flex items-center gap-2 text-sm text-slate-200">
          <input type="checkbox" checked={s.live_fx_enabled} onChange={(e) => setS({ ...s, live_fx_enabled: e.target.checked })} />
          Enable live FX (falls back to manual rates below when off)
        </label>

        <div>
          <div className="text-[10px] uppercase tracking-wider text-slate-500 font-bold mb-1">FX rates (1 base = X)</div>
          <div className="grid grid-cols-3 gap-2">
            {Object.entries(s.fx_rates).map(([code, rate]) => (
              <label key={code} className="block">
                <div className="text-xs text-slate-400 mb-1 font-mono">{code}</div>
                <input type="number" step="0.0001" value={rate} onChange={(e) => setS({ ...s, fx_rates: { ...s.fx_rates, [code]: Number(e.target.value) } })}
