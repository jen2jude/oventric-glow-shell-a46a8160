...
import { Users, Package, ShoppingBag, DollarSign, Megaphone, Flag, Activity, Loader2, RefreshCw } from "lucide-react";
...
function AdminOverview() {
  ...
  const kpis: Array<{ label: string; value: string | number; icon: typeof Users; tint: string; to: string }> = [
    { label: "Users", value: stats.users, icon: Users, tint: "text-blue-300 bg-blue-500/10 border-blue-500/30", to: "/admin/users" },
    { label: "Products", value: stats.products, icon: Package, tint: "text-emerald-300 bg-emerald-500/10 border-emerald-500/30", to: "/admin/products" },
    { label: "Orders", value: stats.orders, icon: ShoppingBag, tint: "text-amber-300 bg-amber-500/10 border-amber-500/30", to: "/admin/products" },
    { label: "Revenue (USD)", value: `$${stats.revenueUsd.toFixed(2)}`, icon: DollarSign, tint: "text-fuchsia-300 bg-fuchsia-500/10 border-fuchsia-500/30", to: "/admin/system-wallets" },
    { label: "Active Campaigns", value: stats.activeCampaigns, icon: Megaphone, tint: "text-cyan-300 bg-cyan-500/10 border-cyan-500/30", to: "/admin/campaigns" },
    { label: "Pending Reports", value: stats.pendingReports, icon: Flag, tint: "text-red-300 bg-red-500/10 border-red-500/30", to: "/admin/reports" },
    { label: "Wallet Txns", value: stats.transactions, icon: Activity, tint: "text-violet-300 bg-violet-500/10 border-violet-500/30", to: "/admin/payouts" },
  ];
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 mb-8">
        {kpis.map((k) => (
          ...
          >
            ...
            <div className="text-white text-2xl font-black group-hover:text-emerald-300 transition-colors">{k.value}</div>
          </Link>
        ))}
      </div>
  ...
  return (
    <div className="bg-[#141418] border border-white/10 rounded-xl p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        {viewAll && (
          <Link to={viewAll} className="text-[11px] text-emerald-300 hover:text-emerald-200 font-semibold">
            View all →
          </Link>
        )}
      </div>
