      ...
      <div className="flex min-h-screen flex-col md:h-full md:min-h-0">
        ...
        <main ref={mainRef} className="flex-1 min-w-0 pb-20 md:overflow-y-auto md:pb-0">
          <div className="max-w-3xl mx-auto w-full px-4 py-6">
            ...
            <div className="profile-lowgpu-top hidden mb-4">
              <div className="profile-lowgpu-actions flex items-center justify-between gap-3 mb-3">
                <button
                  onClick={() => navigate({ to: "/" })}
                  className="inline-flex items-center gap-1.5 px-2 py-2 text-xs font-semibold text-slate-200"
                >
                  <ArrowLeft className="w-3.5 h-3.5" /> Back to feed
                </button>
                <div className="flex items-center gap-2">
                  <button
                    onClick={handleCopyLink}
                    className="inline-flex items-center gap-1.5 px-3 py-2 rounded-md bg-[#1E1E24] border border-white/10 text-xs font-semibold text-slate-200"
                    aria-label="Copy profile link"
                  >
                    <Link2 className="w-3.5 h-3.5 text-slate-300" /> Copy Link
                  </button>
                  {isOwnProfile && (
                    <button
                      onClick={() => setFollowRequestsOpen(true)}
                      className="relative inline-flex items-center gap-1.5 px-3 py-2 rounded-md bg-[#1E1E24] border border-white/10 text-xs font-semibold text-slate-200"
                      aria-label={`Open follow requests${pendingFollowReqCount > 0 ? ` (${pendingFollowReqCount} pending)` : ""}`}
                    >
                      <UserPlus className="w-3.5 h-3.5 text-slate-300" />
                      <span>Requests</span>
                      {pendingFollowReqCount > 0 && (
                        <span className="absolute -top-1 -right-1 min-w-[16px] h-4 px-1 rounded-full bg-[#2a2a30] border border-white/20 text-white text-[9px] font-black flex items-center justify-center" aria-hidden>
                          {pendingFollowReqCount > 9 ? "9+" : pendingFollowReqCount}
                        </span>
                      )}
                    </button>
              ...
              <div className="profile-lowgpu-cover relative mb-3 h-28 border-y border-white/10 bg-[#1E1E24]">
              ...
              <section className="profile-lowgpu-card border border-white/10 bg-[#1E1E24] p-4">
                <div className="flex items-start gap-3">
                  <div className="relative shrink-0">
                    <div className="w-16 h-16 rounded-full bg-[#2A2A30] flex items-center justify-center text-white text-xl font-black overflow-hidden">
                      {displayAvatar ? (
                        <img src={displayAvatar} alt={`${displayName} avatar`} loading="eager" decoding="sync" className="block w-full h-full object-cover" />
                      ) : (
                        displayInitials
                      )}
                    </div>
                    {isOwnProfile && (
                      <button
                        ...
                        className="absolute -bottom-1 -right-1 w-8 h-8 rounded-md bg-[#121214] border border-white/20 text-white flex items-center justify-center"
                      >
                        {uploading === "avatar" ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Camera className="w-4 h-4" strokeWidth={2.4} />}
                      </button>
                    )}
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h1 className="text-white text-2xl font-black leading-tight">{displayName}</h1>
                      <ShieldCheck className="w-4 h-4 text-white" aria-label={displayTierLabel} />
                    </div>
                    <span className="mt-2 inline-flex text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border border-white/40 text-white bg-transparent">
                      {displayTierLabel}
                    </span>
                    <div className="text-sm text-slate-400 mt-2">{displayRole}</div>
                    <div className="text-xs text-slate-500 mt-1">Joined {displayJoined} · ★ {displayStars.toFixed(1)}</div>
                  </div>
                </div>
                <div className="mt-3 flex flex-wrap items-center gap-3 text-xs">
                  <span className="text-slate-300"><span className="font-bold text-white">{(socialCounts?.followers ?? 0).toLocaleString()}</span> <span className="text-slate-500">followers</span></span>
                  <span className="text-slate-300"><span className="font-bold text-white">{(socialCounts?.following ?? 0).toLocaleString()}</span> <span className="text-slate-500">following</span></span>
                  <span className="text-slate-300"><span className="font-bold text-white">{(socialCounts?.circleMembers ?? 0).toLocaleString()}</span> <span className="text-slate-500">in circle</span></span>
                </div>
                ...
                {!isOwnProfile && !identityMissing && realProfile?.userId && (
                  <div className="mt-4 grid grid-cols-2 gap-2">
                    ...
                    <button onClick={handleChat} className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-md border border-white/15 text-white text-sm font-semibold">
                      <MessageCircle className="w-4 h-4" /> Chat
                    </button>
                    <button onClick={() => setJoinCircleOpen(true)} className="col-span-2 inline-flex items-center justify-center gap-2 px-4 py-2 rounded-md border border-white/15 text-white text-sm font-semibold" aria-label="Request to join one of this user's circles">
