...
export const Route = createFileRoute("/admin/management-users")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Create admin & staff accounts and assign role-based access." },
      ...
      {loading ? (
        ...
        <div className="bg-[#141418] border border-white/10 rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {users.length === 0 ? (
                <tr><td colSpan={4} className="px-4 py-8 text-center text-slate-500">No management users yet.</td></tr>
              ) : users.map((u) => (
                <UserRow
                  key={u.userId}
                  user={u}
...
function UserRow({ user, onChanged, onResetPw }: { user: ManagementUserDTO; onChanged: () => void; onResetPw: () => void }) {
  ...
  return (
    <Modal title="New management user" onClose={onClose}>
      <form onSubmit={submit} className="flex flex-col gap-3">
        <Field label="Email">
          <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className={inputCls} placeholder="team@oventric.com" />
        </Field>
        ...
        <Field label="Temporary password (8+ chars)">
          <div className="relative">
            <input type={showPw ? "text" : "password"} required minLength={8} value={password} onChange={(e) => setPassword(e.target.value)} className={inputCls + " pr-16"} />
            <button type="button" onClick={() => setShowPw((v) => !v)} className="absolute right-2 top-1/2 -translate-y-1/2 text-[11px] font-semibold text-slate-400 hover:text-white px-2 py-1">
              {showPw ? "Hide" : "Show"}
            </button>
          </div>
        </Field>
    ...
    <Modal title={`Reset password — ${user.email}`} onClose={onClose}>
      <form onSubmit={submit} className="flex flex-col gap-3">
        <Field label="New password (8+ chars)">
          <div className="relative">
            <input type={showPw ? "text" : "password"} required minLength={8} value={password} onChange={(e) => setPassword(e.target.value)} className={inputCls + " pr-16"} autoFocus />
            <button type="button" onClick={() => setShowPw((v) => !v)} className="absolute right-2 top-1/2 -translate-y-1/2 text-[11px] font-semibold text-slate-400 hover:text-white px-2 py-1">
              {showPw ? "Hide" : "Show"}
            </button>
          </div>
        ...
        {err && <div className="text-xs text-red-400">{err}</div>}
        {ok && <div className="text-xs text-emerald-400 flex items-center gap-1"><ShieldCheck className="w-3.5 h-3.5" /> Password updated.</div>}
        <button
          type="submit"
          disabled={busy || ok}
          className="mt-2 px-4 py-2.5 bg-emerald-500 hover:bg-emerald-400 disabled:opacity-50 text-black text-sm font-bold rounded-lg"
...
const inputCls = "w-full px-3 py-2.5 bg-[#0b0b0d] border border-white/10 rounded-lg text-sm text-white placeholder:text-slate-500 focus:outline-none focus:border-emerald-500/50";
...
function Modal({ title, children, onClose }: { title: string; children: React.ReactNode; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" onClick={onClose}>
      <div className="w-full max-w-lg bg-[#141418] border border-white/10 rounded-2xl p-6" onClick={(e) => e.stopPropagation()}>
