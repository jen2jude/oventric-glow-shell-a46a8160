...
import {
  ...
  type MyCampaignDetail, type MyCampaignMetrics, type MyCampaignLead,
...
function CampaignDetailPage() {
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "ad_events", filter: `campaign_id=eq.${id}` }, () => { void refresh(); })
      .on("postgres_changes", { event: "*", schema: "public", table: "ad_leads", filter: `campaign_id=eq.${id}` }, () => { void refresh(); })
      .on("postgres_changes", { event: "*", schema: "public", table: "ad_daily_spend", filter: `campaign_id=eq.${id}` }, () => { void refresh(); })
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "ad_campaigns", filter: `id=eq.${id}` }, () => { void refresh(); })
      .subscribe();
    ...
    return (
      <div className="page-light min-h-screen bg-[#0b0b0d] flex items-center justify-center p-6">
        <div className="max-w-md text-center">
          ...
          <button onClick={() => navigate({ to: "/ads-manager" })} className="rounded-lg bg-emerald-500 text-black px-4 py-2 text-sm font-semibold">Back to Ads Manager</button>
        </div>
      </div>
    );
  }
    ...
    <div className="page-light min-h-screen bg-[#0b0b0d] text-slate-200 pb-24">
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <header className="mb-4">
          ...
          <div className="text-[11px] uppercase tracking-widest text-slate-500 mt-1">View only · Contact support to edit</div>
        </header>

        {/* KPI cards */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
          <Kpi icon={Eye} label="Impressions" value={(metrics?.totals.impressions ?? 0).toLocaleString()} />
          <Kpi icon={MousePointerClick} label="Clicks" value={(metrics?.totals.clicks ?? 0).toLocaleString()} sub={`CTR ${metrics?.ctr.toFixed(2) ?? "0.00"}%`} />
          <Kpi icon={Users} label="Reach (est.)" value={(metrics?.reach_estimate ?? 0).toLocaleString()} />
          <Kpi icon={DollarSign} label="Spent" value={`$${(metrics?.totals.spent ?? 0).toFixed(2)}`} sub={`CPM $${(metrics?.cpm ?? 0).toFixed(2)}`} />
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-3 mt-5">
          <Section title="Setup" icon={Target}>
            ...
            {detail.cta_type === "whatsapp" && <Row label="WhatsApp" value={detail.cta_whatsapp || "—"} />}
          </Section>

          <Section title="Targeting & Schedule" icon={MapPin}>
            <Row label="Countries" value={detail.countries.join(", ") || "All"} />
            <Row label="Cities" value={detail.cities.length ? `${detail.cities.length} cities` : "All"} multiline />
        ...
        <div className="mt-5">
          <Section title="Last 30 days" icon={PieChart}>
            ...
            <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-2">
              ...
              <MiniStat label="WhatsApp clicks" value={metrics?.whatsapp_clicks ?? 0} icon={MessageCircle} />
...
function MiniStat({ label, value, icon: Icon }: { label: string; value: number | string; icon: React.ComponentType<{ className?: string }> }) {
  return (
    <div className="rounded-lg bg-black/20 border border-white/5 p-2">
      ...
      <div className="text-white font-black text-sm mt-0.5">{typeof value === "number" ? value.toLocaleString() : value}</div>
    </div>
  );
}

function EscrowCard({ detail }: { detail: MyCampaignDetail }) {
  const pctSpent = detail.escrow_locked > 0
  ...
  return (
    <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/5 p-4">
      <div className="flex items-start justify-between gap-2 flex-wrap">
        <div>
          <div className="text-[10px] uppercase tracking-widest text-emerald-400 font-bold">Wallet Escrow</div>
          <div className="text-white text-lg font-black mt-1">${detail.escrow_locked.toFixed(2)} locked from your wallet</div>
        </div>
        <div className="text-right">
          <div className="text-[10px] uppercase tracking-widest text-slate-500 font-bold">Balance left</div>
          <div className="text-emerald-300 text-lg font-black">${detail.escrow_remaining.toFixed(2)}</div>
        </div>
      </div>
...
function Row({ label, value, multiline }: { label: string; value: string; multiline?: boolean }) {
  return (
    <div className="grid grid-cols-[110px_minmax(0,1fr)] gap-2 text-sm">
      <div className="text-slate-500 text-[11px] uppercase tracking-widest font-bold pt-0.5">{label}</div>
      <div className={`text-slate-200 ${multiline ? "whitespace-pre-wrap break-words" : "truncate"}`}>{value}</div>
    </div>
  );
}
  ...
  return (
    <div className="grid grid-cols-[110px_minmax(0,1fr)] gap-2 text-sm">
      ...
      <div className="flex items-center gap-2 min-w-0">
        ...
        {value && (
          <>
            <button onClick={copy} className="text-slate-400 hover:text-white shrink-0" title="Copy"><Copy className="w-3.5 h-3.5" /></button>
            <a href={value} target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-white shrink-0" title="Open"><ExternalLink className="w-3.5 h-3.5" /></a>
          </>
        )}
      </div>
    </div>
...
function LeadsTable({ leads }: { leads: MyCampaignLead[] | null }) {
  ...
  if (leads.length === 0) return <div className="text-slate-500 text-sm">No leads yet. New submissions will appear here instantly.</div>;
          ...
          {leads.map((l) => (
            <tr key={l.id} className="hover:bg-white/5">
              ...
              <td className="px-4 py-2">
                {l.email ? (
                  <button onClick={() => copy(l.email!)} className="text-emerald-300 hover:underline inline-flex items-center gap-1">
              ...
              <td className="px-4 py-2">
                {l.phone ? (
                  <button onClick={() => copy(l.phone!)} className="text-emerald-300 hover:underline inline-flex items-center gap-1">
                    {l.phone} <Copy className="w-3 h-3 opacity-60" />
                  </button>
                ) : "—"}
              </td>
              <td className="px-4 py-2 text-slate-300 max-w-[280px] truncate">{l.message || "—"}</td>
              <td className="px-4 py-2 text-slate-500 whitespace-nowrap">{fmtDate(l.created_at)}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function MetricsChart({ metrics }: { metrics: MyCampaignMetrics | null }) {
  ...
  const max = useMemo(() => Math.max(1, ...series.map((s) => Math.max(s.impressions, s.clicks * 10))), [series]);
  if (series.length === 0) {
    return <div className="h-32 rounded-lg bg-black/20 border border-white/5 flex items-center justify-center text-slate-500 text-sm">No delivery data yet</div>;
  }
  return (
    <div>
      <div className="h-40 flex items-end gap-[2px] rounded-lg bg-black/20 border border-white/5 p-2">
        {series.map((s) => {
          ...
          return (
            <div key={s.day} className="flex-1 h-full flex flex-col justify-end gap-[1px]" title={`${s.day}: ${s.impressions} imp / ${s.clicks} clk / $${s.spent_usd}`}>
      ...
      <div className="flex gap-4 mt-2 text-[11px] text-slate-400">
        <span className="inline-flex items-center gap-1"><span className="w-2 h-2 bg-emerald-500/70 rounded-sm" /> Clicks (×10)</span>
        <span className="inline-flex items-center gap-1"><span className="w-2 h-2 bg-emerald-500/25 rounded-sm" /> Impressions</span>
      </div>
    </div>
  );
}
