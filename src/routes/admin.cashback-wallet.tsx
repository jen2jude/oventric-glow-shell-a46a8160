...
export const Route = createFileRoute("/admin/cashback-wallet")({
  head: () => ({ meta: [{ title: "Cashback Wallet · Admin" }, { name: "robots", content: "noindex, nofollow" }] }),
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        setSummary(s); setUsers(u); setHistory(h);
      } catch (e) {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <header className="mb-6">
        ...
        <p className="text-sm text-slate-400">Every user's Cashback Wallet balance and how they earned it (2% of each marketplace / academy sale).</p>
      </header>

      {err && <div className="mb-4 text-sm text-red-300 bg-red-500/10 border border-red-500/40 rounded-lg p-3">{err}</div>}

      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <SummaryCard label="Outstanding" value={fmt(summary?.totalOutstandingUSD ?? 0)} sub="Owed to users right now" icon={Gift} tone="from-pink-500/25 to-fuchsia-700/10 border-pink-500/30" />
        <SummaryCard label="Total Earned" value={fmt(summary?.totalEarnedUSD ?? 0)} sub="All-time cashback credited" icon={TrendingUp} tone="from-emerald-500/25 to-teal-700/10 border-emerald-500/30" />
        <SummaryCard label="Total Spent" value={fmt(summary?.totalSpentUSD ?? 0)} sub="Applied to purchases" icon={ArrowDownRight} tone="from-sky-500/25 to-indigo-700/10 border-sky-500/30" />
        <SummaryCard label="Active Wallets" value={String(summary?.userCount ?? 0)} sub="Users with balance > $0" icon={Users} tone="from-amber-500/25 to-orange-700/10 border-amber-500/30" />
      </div>
        ...
        <div className="bg-[#141418] border border-white/10 rounded-xl overflow-hidden">
          ...
          <div className="divide-y divide-white/5 max-h-[560px] overflow-y-auto">
            {!users ? (
              ...
              <div className="p-6 text-sm text-slate-500 text-center">No cashback balances yet.</div>
            ) : users.map((u) => (
              <div key={u.userId} className="p-4 flex items-center justify-between text-sm">
                <div className="min-w-0">
                  <div className="text-white font-semibold truncate">{u.displayName || u.username || "Unnamed user"}</div>
                  <div className="text-[11px] text-slate-500 font-mono truncate">{u.username ? `@${u.username}` : u.userId.slice(0, 8)}</div>
                </div>
                <div className="text-emerald-300 font-mono font-bold whitespace-nowrap">{fmt(u.accumulatedUSD)}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="divide-y divide-white/5 max-h-[560px] overflow-y-auto">
            {!history ? (
              ...
              <div key={h.id} className="p-4 flex items-center justify-between text-sm">
                <div className="min-w-0">
                  <div className="text-white font-semibold truncate">{h.displayName || h.username || h.userId.slice(0, 8)}</div>
                  <div className="text-[11px] text-slate-500 font-mono truncate">{h.txHash} · {new Date(h.occurredAt).toLocaleString()}</div>
                </div>
                <div className={`font-mono font-bold whitespace-nowrap ${h.inflow ? "text-emerald-300" : "text-rose-300"}`}>
...
function SummaryCard({ label, value, sub, icon: Icon, tone }: { label: string; value: string; sub: string; icon: React.ComponentType<{ className?: string }>; tone: string }) {
  return (
