...
const STATUSES: (PayoutStatus | "ALL")[] = ["pending", "approved", "paid", "rejected", "cancelled", "ALL"];
    ...
    <div className="p-4 sm:p-6 space-y-4">
      <header className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-slate-400">Review, approve, reject and settle user withdrawals.</p>
        </div>
      </header>
...
function PayoutRow({
  ...
  const Icon =
    p.status === "paid" ? CheckCircle2 :
    p.status === "rejected" ? XCircle :
    p.status === "cancelled" ? AlertTriangle : Clock3;
  ...
  return (
    <div className="border-t border-[#1c1c20] first:border-t-0">
      <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 p-4">
        <div className="min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <span className={`inline-flex items-center gap-1 rounded-md border px-2 py-0.5 text-[11px] font-bold uppercase ${tone[p.status]}`}>
              <Icon className="w-3 h-3" /> {p.status}
            </span>
            <span className="text-sm font-bold text-white tabular-nums">
              {sym}{Number(p.amount).toLocaleString("en-US", { minimumFractionDigits: p.currency === "NGN" ? 0 : 2 })} {p.currency}
            </span>
          ...
          <div className="mt-1 text-xs text-slate-400 truncate">
            {p.requester_name ?? p.requester_username ?? p.user_id.slice(0, 8)} · {new Date(p.created_at).toLocaleString()}
          </div>
        </div>
        <button
          onClick={onToggle}
        ...
        <div className="px-4 pb-4 space-y-3">
          <div className="rounded-lg border border-[#222226] bg-[#0A0A0C] p-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
              {Object.entries(p.destination ?? {}).map(([k, v]) => (
                <div key={k} className="grid grid-cols-[110px_minmax(0,1fr)_auto] items-center gap-2">
          ...
          {(p.admin_note || p.reject_reason) && (
            <div className="text-xs text-slate-400">
              {p.admin_note && <div><span className="text-slate-500">Note:</span> {p.admin_note}</div>}
              {p.reject_reason && <div><span className="text-slate-500">Reason:</span> {p.reject_reason}</div>}
