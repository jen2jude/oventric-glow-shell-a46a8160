...
import { Plus, Loader2, Edit3, Trash2, AlertCircle, GraduationCap, Sparkles, Eye, EyeOff } from "lucide-react";
  ...
  return (
    <div className="p-8 flex items-start gap-3 text-slate-300">
      ...
      <div>
        ...
        <button onClick={() => { reset(); router.invalidate(); }} className="mt-3 px-3 py-1.5 rounded bg-emerald-500 text-black text-sm font-bold">
          Retry
        </button>
      </div>
    </div>
...
function AdminCourses() {
  ...
  const filtered = (rows ?? []).filter((r) =>
    !q ? true : `${r.title} ${r.instructorName} ${r.category}`.toLowerCase().includes(q.toLowerCase())
  );
    ...
    <div className="p-6 max-w-6xl">
      <div className="flex items-center justify-between gap-3 flex-wrap mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Publish, edit, and manage every course across the platform.</p>
        </div>
        <div className="flex gap-2">
          <Link to="/" className="px-3 py-2 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 text-slate-300 text-sm">View catalog</Link>
          <button
        ...
        {filtered.map((c) => (
          <div key={c.id} className="p-4 bg-[#1E1E24] border border-white/10 rounded-xl flex items-center gap-4">
            <div className="w-16 h-10 rounded bg-black/40 overflow-hidden shrink-0 flex items-center justify-center">
              {c.coverUrl ? (
                <ResponsiveImage sizes="(min-width: 640px) 240px, 50vw" src={c.coverUrl} alt="" className="w-full h-full object-cover"  loading="lazy" decoding="async" />
              ) : (
            ...
            <div className="flex items-center gap-1 shrink-0">
              <button onClick={() => togglePublish(c)} title={c.isPublished ? "Unpublish" : "Publish"} className="p-2 rounded hover:bg-white/5 text-slate-400 hover:text-white">
                {c.isPublished ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}
              </button>
              <button onClick={() => togglePromote(c)} title={c.promoted ? "Unfeature" : "Feature"} className={`p-2 rounded hover:bg-white/5 ${c.promoted ? "text-emerald-300" : "text-slate-400 hover:text-white"}`}>
                <Sparkles className="w-4 h-4" />
              </button>
              <button onClick={() => { setEditingId(c.id); setEditorOpen(true); }} title="Edit" className="p-2 rounded hover:bg-white/5 text-slate-400 hover:text-white">
                <Edit3 className="w-4 h-4" />
              </button>
              <button onClick={() => del(c.id)} title="Delete" className="p-2 rounded hover:bg-red-500/10 text-red-400">
      ...
      <CourseEditorModal
        ...
        onSaved={() => { setEditorOpen(false); setRefresh((k) => k + 1); }}
      />
    </div>
  );
}
