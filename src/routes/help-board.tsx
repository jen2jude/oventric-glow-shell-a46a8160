...
import {
  Receipt, MessageSquareWarning, Target, ShoppingBag, Wallet, ShieldAlert,
  ChevronDown, Star, MessageCircle, Send, Check, X,
} from "lucide-react";
...
export const Route = createFileRoute("/help-board")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Open a dispute, share feedback, browse FAQs or start a live chat with the Oventric support team, any time of day." },
      { property: "og:title", content: "Oventric Help Board" },
      { property: "og:description", content: "24/7 support: disputes, feedback, FAQs and live chat with our team." },
...
const DISPUTES = [
  { key: "transaction", label: "Transaction dispute", icon: Receipt, tint: "text-sky-300 bg-sky-500/15" },
  { key: "social_post", label: "Social post dispute", icon: MessageSquareWarning, tint: "text-violet-300 bg-violet-500/15" },
  { key: "bounty", label: "Bounties dispute", icon: Target, tint: "text-amber-300 bg-amber-500/15" },
  { key: "marketplace", label: "Marketplace dispute", icon: ShoppingBag, tint: "text-emerald-300 bg-emerald-500/15" },
...
const FAQS = [
  { q: "How does escrow protect my purchase?", a: "Digital purchases are held in escrow. Funds release to the seller when you confirm delivery, or automatically 48 hours after delivery is marked if you take no action." },
  { q: "When do I get my cashback?", a: "2% cashback is credited to your cashback wallet right after a successful purchase. It can be spent on digital products and courses." },
  { q: "Why is my price shown in a different currency?", a: "Every listing is converted to your home currency using near-live FX rates, and you're charged in that currency." },
  { q: "How long do payouts take?", a: "Automated payouts to supported banks and mobile money usually land within minutes. Manual USD payouts are reviewed by our team." },
  { q: "How do bounty payments work?", a: "The reward is escrowed when the bounty is published. Once you accept the delivered work, the solver is paid instantly." },
  { q: "Can I change my country or currency?", a: "Your wallet currency is tied to your country. Contact support through live chat if you have relocated." },
];
...
function HelpBoardPage() {
  ...
  const sendTicket = async () => {
    ...
    setTicketBusy(true);
    try {
      await ticketFn({ data: { category: activeDispute.key, subject: subject.trim(), details: details.trim() } });
      setTicketDone(true);
      setOpenDispute(null);
          ...
          {feedbackOpen && (
            <div className="mt-2 p-4 rounded-2xl bg-[#141418] border border-white/10 md:bg-white md:border-slate-200">
              ...
              <div className="mt-3 flex items-center gap-1.5">
                {[1, 2, 3, 4, 5].map((n) => (
                  <button key={n} onClick={() => setRating(n)} aria-label={`${n} star`}>
                    <Star className={`w-6 h-6 ${n <= rating ? "text-amber-300 fill-amber-300" : "text-slate-600"}`} />
                  </button>
                ))}
              </div>
              <textarea
              ...
              <div className="mt-3 flex items-center gap-3">
                ...
                {feedbackDone && <span className="text-sm text-emerald-300">Thanks for the feedback!</span>}
              </div>
            </div>
          )}
        </section>
