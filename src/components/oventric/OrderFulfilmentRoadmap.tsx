  ...
  return (
    <div className="rounded-[10px] border border-white/10 md:border-slate-200 bg-[#1E1E24] md:bg-white p-4">
      ...
      {data.escrowStatus === "held" && auto && data.deliveredAt && (
        <div className="flex items-center gap-2 text-[11px] text-amber-200 bg-amber-500/5 border border-amber-500/30 rounded-[10px] px-3 py-2 mb-3">
      ...
      {data.role === "seller" && data.deliveredAt && data.escrowStatus === "held" && (
        <div className="text-[11px] text-slate-400 md:text-slate-500 bg-[#121214] md:bg-slate-50 border border-white/10 md:border-slate-200 rounded-[10px] px-3 py-2 mb-3">
      ...
      <div className="rounded-[10px] border border-emerald-500/30 bg-emerald-500/5 px-3 py-2 mb-3 text-[11px] text-emerald-100 leading-relaxed">
      ...
      <div className="flex flex-wrap gap-2">
        {canDeliver && (
          <button
            ...
            className="inline-flex items-center gap-2 px-3.5 py-2 rounded-[10px] text-sm font-bold text-black"
        ...
        <button
          onClick={contactPeer}
          className="inline-flex items-center gap-2 px-3.5 py-2 rounded-[10px] text-sm font-semibold text-white md:text-slate-900 bg-[#2A2A31] md:bg-slate-100 border border-white/10 md:border-slate-200"
        >
        ...
        {canDispute && (
          <button
            ...
            className="inline-flex items-center gap-2 px-3.5 py-2 rounded-[10px] text-sm font-semibold text-red-300 bg-[#2A2A31] md:bg-slate-100 border border-red-500/40"
          >
          ...
          {confirmModal === "deliver" && (
            <label className="block mb-4">
              ...
              <textarea
                ...
                className="w-full rounded-[10px] bg-[#121214] md:bg-slate-50 border border-white/10 md:border-slate-200 px-3 py-2 text-sm text-white md:text-slate-900 placeholder:text-slate-600"
              />
            </label>
          )}
        </ConfirmModal>
...
function StepNode({ step }: { step: FulfilmentStep }) {
  ...
  return (
    <div className="flex items-center gap-2 min-w-0 flex-1 rounded-[10px] px-2 py-2 bg-[#121214] md:bg-slate-50 border border-white/10 md:border-slate-200">
      <span
    ...
    <div className="modal-light fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/70">
      <div className="w-full max-w-sm rounded-xl border border-white/10 md:border-slate-200 bg-[#1E1E24] md:bg-white p-5">
        ...
        <div className="flex gap-2 justify-end">
          <button
            onClick={onCancel}
            className="px-3 py-2 rounded-[10px] text-sm text-slate-300 md:text-slate-600 bg-[#2A2A31] md:bg-slate-100 border border-white/10 md:border-slate-200"
          >
            Cancel
          </button>
          <button
            onClick={onConfirm}
            disabled={busy}
            className="px-4 py-2 rounded-[10px] text-sm font-bold text-black disabled:opacity-60 inline-flex items-center gap-2"
            style={{ backgroundColor: "#3b82f6" }}
          >
    ...
    <div className="modal-light fixed inset-0 z-[80] flex items-end sm:items-center justify-center bg-black/70 p-0 sm:p-4">
      <div className="w-full sm:max-w-lg max-h-[92vh] overflow-y-auto rounded-t-2xl sm:rounded-xl border border-white/10 md:border-slate-200 bg-[#1E1E24] md:bg-white p-5">
        ...
        <select
          ...
          className="w-full mb-3 rounded-[10px] bg-[#121214] md:bg-slate-50 border border-white/10 md:border-slate-200 px-3 py-2 text-sm text-white md:text-slate-900"
        >
          {REASONS.map((r) => (
        ...
        <textarea
          ...
          className="w-full mb-3 rounded-[10px] bg-[#121214] md:bg-slate-50 border border-white/10 md:border-slate-200 px-3 py-2 text-sm text-white md:text-slate-900 placeholder:text-slate-600"
        />

        <label className="block text-[11px] uppercase tracking-widest text-slate-500 mb-1">
          Evidence (up to 5 images)
        </label>
        <label className="inline-flex items-center gap-2 px-3 py-2 rounded-[10px] text-sm text-slate-200 md:text-slate-700 bg-[#2A2A31] md:bg-slate-100 border border-white/10 md:border-slate-200 cursor-pointer mb-3">
        ...
        <div className="flex justify-end gap-2">
          ...
          <button
            onClick={submit}
            disabled={busy}
            className="px-4 py-2 rounded-[10px] text-sm font-bold text-white md:text-slate-900 disabled:opacity-60 inline-flex items-center gap-2"
