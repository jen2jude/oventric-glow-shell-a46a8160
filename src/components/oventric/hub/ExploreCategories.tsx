    ...
    <div className="grid grid-cols-3 gap-2">
      {categories.map((cat) => (
        <button
          ...
          className="group flex flex-col items-center justify-center aspect-[1.1] gap-2 rounded-[10px] bg-[#141416] border border-white/[0.06] transition-all active:scale-95 hover:bg-[#1A1A1E]"
        >
          <div className={`flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br ${cat.color} ${cat.glow} group-hover:scale-105 transition-transform`}>
            <cat.icon className="h-5 w-5 text-white" />
          </div>
          <span className="text-[10px] font-bold uppercase tracking-tight text-white/60 group-hover:text-white transition-colors">
            {cat.name}
          </span>
        </button>
      ))}
