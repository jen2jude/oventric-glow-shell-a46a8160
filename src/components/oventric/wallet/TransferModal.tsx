...
import { searchTransferRecipients, transferToUser, type TransferRecipientDTO } from "@/lib/wallet.functions";
  ...
  const submit = async () => {
    ...
    try {
      const res = await doTransfer({
        data: { recipientId: recipient.userId, currency: baseCurrency, amount: numericAmount, note: note || undefined },
      });
      setResult({ ok: true, message: `Sent ${formatMoney(numericAmount, baseCurrency)} to ${res.recipient_name}.` });
      setStep("result");
      onDone();
    } catch (e) {
    ...
    return (
      <ModalShell title={result.ok ? "Transfer Sent" : "Transfer Failed"} onClose={onClose}>
        <div className="text-center py-4">
          <div className={`mx-auto w-14 h-14 rounded-full flex items-center justify-center mb-3 ${result.ok ? "bg-emerald-500/15 text-emerald-300" : "bg-red-500/15 text-red-300"}`}>
            <CheckCircle2 className="w-7 h-7" />
          </div>
          <p className="text-sm text-slate-200 md:text-slate-700">{result.message}</p>
        </div>
        <button onClick={onClose} className="w-full rounded-xl bg-emerald-500 hover:bg-emerald-400 text-black font-bold py-2.5 text-sm">
          Done
        </button>
      </ModalShell>
    );
    ...
    return (
      <ModalShell title="Send to User" onClose={onClose}>
        ...
        <div className="space-y-1.5 min-h-[80px]">
          {searching && <div className="text-xs text-slate-500 py-2 inline-flex items-center gap-2"><Loader2 className="w-3.5 h-3.5 animate-spin" /> Searching…</div>}
          ...
          {results.map((r) => (
            ...
            >
              ...
              <div className="min-w-0">
                <div className="text-sm font-semibold text-white md:text-slate-900 truncate">{r.displayName ?? r.username ?? "User"}</div>
                <div className="text-[11px] text-slate-500 truncate">@{r.username ?? "unknown"}</div>
              </div>
            </button>
          ))}
        </div>
  ...
  if (step === "amount" && recipient) {
    ...
    return (
      <ModalShell title={`Send to @${recipient.username ?? "user"}`} onClose={onClose}>
        <button onClick={() => setStep("recipient")} className="text-[11px] text-slate-400 uppercase tracking-wider">← Change recipient</button>
        <div>
          <label className="text-[11px] uppercase tracking-wider text-slate-400 font-semibold">Amount ({baseCurrency})</label>
          <div className="mt-1 grid grid-cols-[auto_minmax(0,1fr)] items-center rounded-xl border border-[#222226] md:border-slate-200 bg-[#0A0A0C] md:bg-white focus-within:border-fuchsia-500/60">
            ...
            <input
              type="number" min={0} step={step_} inputMode="decimal" value={amount}
              onChange={(e) => setAmount(e.target.value)}
              className="w-full bg-transparent py-2.5 pr-3 text-sm text-white md:text-slate-900 outline-none tabular-nums"
            />
          </div>
          <div className="mt-1 text-[11px] text-slate-500">Available: {formatMoney(available, baseCurrency)}</div>
        </div>
        <div>
          <label className="text-[11px] uppercase tracking-wider text-slate-400 font-semibold">Note (optional)</label>
          <input
            value={note} onChange={(e) => setNote(e.target.value)} maxLength={200} placeholder="What's this for?"
    ...
    return (
      <ModalShell title="Confirm Transfer" onClose={onClose}>
        <div className="rounded-xl border border-[#222226] md:border-slate-200 bg-[#0A0A0C] md:bg-white p-4 space-y-2 text-sm">
          <div className="flex justify-between"><span className="text-slate-500">To</span><span className="font-semibold text-white md:text-slate-900">@{recipient.username ?? "user"}</span></div>
          <div className="flex justify-between"><span className="text-slate-500">Amount</span><span className="font-semibold text-white md:text-slate-900">{formatMoney(numericAmount, baseCurrency)}</span></div>
          {note && <div className="flex justify-between gap-4"><span className="text-slate-500">Note</span><span className="text-slate-300 md:text-slate-600 truncate">{note}</span></div>}
          <div className="flex justify-between border-t border-[#222226] md:border-slate-200 pt-2"><span className="text-slate-500">No fee</span><span className="text-emerald-300 font-semibold">Instant</span></div>
        </div>
        <div className="flex gap-2">
          <button onClick={() => setStep("amount")} className="flex-1 rounded-xl border border-[#222226] md:border-slate-200 py-2.5 text-sm text-slate-300 md:text-slate-600">Back</button>
          <button
            onClick={submit}
            disabled={busy}
          ...
          >
            {busy ? <><Loader2 className="w-4 h-4 animate-spin" /> Sending…</> : "Confirm & Send"}
          </button>
        </div>
      </ModalShell>
    );
