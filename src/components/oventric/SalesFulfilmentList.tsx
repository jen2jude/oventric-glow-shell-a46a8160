...
export function saleBadge(s: SaleDTO): { label: string; cls: string } {
  if (s.disputeStatus === "open") return { label: "Disputed", cls: "bg-red-500/15 text-red-300 md:text-red-700" };
  if (s.escrowStatus === "released") return { label: "Settled", cls: "bg-emerald-500/15 text-emerald-300 md:text-emerald-700" };
  if (s.deliveredAt) return { label: "Awaiting buyer", cls: "bg-amber-500/15 text-amber-300 md:text-amber-700" };
  if (overdue(s)) return { label: "Overdue 24h+", cls: "bg-red-500 text-white" };
  if (s.requiresManualDelivery) return { label: "Deliver now", cls: "bg-white text-black md:bg-slate-900 md:text-white" };
...
export function SalesFulfilmentList({
  ...
  const counts = useMemo(() => {
    const c: Record<SaleFilter, number> = { all: rows.length, deliver: 0, awaiting: 0, disputed: 0, settled: 0 };
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex gap-1.5 overflow-x-auto no-scrollbar" role="tablist" aria-label="Order filters">
          {FILTERS.map((f) => {
      ...
      {filtered.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-white/15 md:border-slate-300 p-6 text-center">
          ...
          <p className="text-xs text-slate-500 mt-1">Try a different filter or clear your search.</p>
        </div>
      ) : (
            ...
            <>
              ...
              <div className="mt-3 space-y-4">
                <div className="rounded-xl border border-white/10 md:border-slate-200 bg-white/5 md:bg-slate-50 p-3 space-y-1.5 text-xs">
                  ...
                  <Row label="Gross" value={formatMoney(openSale.displayTotal, openSale.displayCurrency)} />
                  ...
                  <Row
                    label="Delivered"
                    value={openSale.deliveredAt ? new Date(openSale.deliveredAt).toLocaleString() : "Not yet"}
                  />
                  <Row
                    label="Auto-release"
                    value={openSale.autoReleaseAt ? new Date(openSale.autoReleaseAt).toLocaleString() : "—"}
                  />
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-3">
      ...
      <span className="font-semibold text-white md:text-slate-900 truncate max-w-[60%] text-right">{value}</span>
    </div>
  );
}
