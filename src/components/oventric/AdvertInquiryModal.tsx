...
export function AdvertInquiryModal({
...
}: { open: boolean; onClose: () => void; initialTier?: Tier }) {
  ...
  const uploadImage = async (file: File) => {
    if (imagePaths.length >= 5) { toast.error("Max 5 images"); return; }
    ...
    try {
      ...
      const { error } = await supabase.storage.from("ad-media").upload(key, file, { upsert: false });
  ...
  const canGoNext = () => {
    ...
    if (step === 3) return header.trim().length > 0 && (ctaType !== "website" || ctaUrl.trim().length > 0);
    if (step === 4) return countries.length > 0 && duration >= 1 && dailyBudget >= TIER_MIN_DAILY[tier];
    return false;
  };

  const openConfirm = () => {
    if (!isAuthenticated) { openGate("generic"); return; }
    if (!canGoNext()) { toast.error("Please complete the required fields"); return; }
    setConfirmOpen(true);
  };
      ...
      <div className="relative w-full sm:max-w-3xl max-h-[92vh] overflow-y-auto bg-[#141418] border border-white/10 sm:rounded-2xl rounded-t-2xl">
        <div className="sticky top-0 z-10 flex items-center justify-between px-5 h-14 bg-[#141418]/95 border-b border-white/10">
          <div>
            ...
            <div className="text-[10px] uppercase tracking-wider text-emerald-300">Step {step} of 4</div>
          </div>
          <button onClick={onClose} aria-label="Close" className="p-2 rounded-lg hover:bg-white/5"><X className="w-5 h-5 text-slate-400" /></button>
        </div>
        ...
        {done ? (
          <div className="p-8 text-center">
            ...
            <p className="text-sm text-slate-400 mt-2 max-w-md mx-auto">
              An admin will contact you shortly to confirm your creative and total budget.
              You'll be asked to fund your wallet with <span className="text-emerald-300 font-bold">${total.toFixed(2)}</span> before your campaign goes live.
            </p>
            <button onClick={onClose} className="mt-6 px-6 py-2.5 bg-emerald-500 hover:bg-emerald-400 text-black text-sm font-black rounded-full">Done</button>
          </div>
        ) : (
          <div className="p-5 space-y-5">
              ...
              <>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(["text", "image", "video"] as Tier[]).map((t) => (
                    <button
                      key={t}
                      onClick={() => { setTier(t); setDailyBudget(Math.max(dailyBudget, TIER_MIN_DAILY[t])); }}
                    ...
                    >
                      {t}
                      <div className="text-[10px] text-slate-500 mt-1">${TIER_MIN_DAILY[t]}/day min</div>
                    </button>
                  ))}
                </div>
            ...
            {step === 2 && (
              <>
                ...
                <Label>Email *</Label>
                <input className="input" type="email" value={contactEmail} onChange={(e) => setContactEmail(e.target.value)} placeholder="you@company.com" />
                <div className="grid grid-cols-2 gap-3">
                  <div><Label>Phone / WhatsApp</Label><input className="input" value={contactPhone} onChange={(e) => setContactPhone(e.target.value)} placeholder="+234…" /></div>
                  <div><Label>Company</Label><input className="input" value={company} onChange={(e) => setCompany(e.target.value)} placeholder="Optional" /></div>
                </div>
                <Label>Website</Label>
            ...
            {step === 3 && (
              <>
                ...
                <input className="input" maxLength={60} value={header} onChange={(e) => setHeader(e.target.value)} placeholder="Grab attention in one line" />
                <Label>Short description (max 140 chars)</Label>
                <input className="input" maxLength={140} value={description} onChange={(e) => setDescription(e.target.value)} placeholder="One-sentence hook" />
                {(tier === "video" || tier === "text") && (
                  <>
                    <Label>Body (max 500 chars)</Label>
                    <textarea className="input min-h-[80px]" maxLength={500} value={body} onChange={(e) => setBody(e.target.value)} placeholder="Longer message shown with your ad" />
                  </>
                )}
                ...
                <div className="grid grid-cols-3 gap-2">
                  {CTA_TYPES.map((c) => (
                    <button key={c.id} onClick={() => setCtaType(c.id)} className={`p-2 rounded-lg border text-xs font-bold ${ctaType === c.id ? "border-emerald-500 bg-emerald-500/10 text-emerald-200" : "border-white/10 bg-[#0f0f12] text-slate-300"}`}>
                      {c.label}
                    </button>
                  ))}
                </div>
                {ctaType === "whatsapp" ? (
                  <input className="input" value={ctaWhatsapp} onChange={(e) => setCtaWhatsapp(e.target.value)} placeholder="WhatsApp number e.g. +2348012345678" />
                ) : (
                  <input className="input" value={ctaUrl} onChange={(e) => setCtaUrl(e.target.value)} placeholder="Destination URL https://…" />
                )}
                ...
                {tier === "image" && (
                  <>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {imagePaths.map((p) => (
                        <div key={p} className="relative w-20 h-20 rounded-lg bg-[#0f0f12] border border-white/10 grid place-items-center text-[10px] text-slate-500">
                          <span className="truncate px-1">{p.split("/").pop()}</span>
                          <button onClick={() => setImagePaths((arr) => arr.filter((x) => x !== p))} className="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-red-500 text-white grid place-items-center">
                  ...
                  <>
                    ...
                    {videoPath ? (
                      <div className="flex items-center gap-2 p-3 rounded-lg bg-[#0f0f12] border border-white/10">
                        ...
                        <span className="flex-1 text-xs text-slate-300 truncate">{videoPath.split("/").pop()}</span>
                        <button onClick={() => setVideoPath("")} className="text-red-400 text-xs font-bold">Remove</button>
                      </div>
                    ) : (
                      <label className="flex items-center gap-2 p-3 rounded-lg border-2 border-dashed border-white/10 cursor-pointer hover:border-emerald-500/50 text-xs text-slate-400">
                        ...
                        <input type="file" accept="video/mp4,video/webm" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) uploadVideo(f); e.target.value = ""; }} />
                      </label>
                    )}
                    <input className="input" value={videoUrl} onChange={(e) => setVideoUrl(e.target.value)} placeholder="Or paste a YouTube / Vimeo link" />
                  </>
                )}
              </>
            )}
            ...
            {step === 4 && (
              <>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div><Label>Age range</Label><select className="input" value={ageRange} onChange={(e) => setAgeRange(e.target.value)}>
                    <option>18-24</option><option>18-34</option><option>18-45</option><option>25-45</option><option>25-55</option><option>all</option>
                  </select></div>
                  <div><Label>Gender</Label><select className="input" value={gender} onChange={(e) => setGender(e.target.value as "all" | "male" | "female")}>
                ...
                <textarea className="input min-h-[70px]" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Anything else our team should know?" />
              </>
            )}
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              {step < 4 ? (
                <button
                  onClick={() => canGoNext() ? setStep((s) => (s + 1) as 2 | 3 | 4) : toast.error("Please fill the required fields")}
          ...
          <div className="absolute inset-0 z-20 grid place-items-center bg-black/80 p-5">
            <div className="w-full max-w-md rounded-2xl bg-[#1a1a1f] border border-emerald-500/30 p-6">
              ...
              <p className="text-sm text-slate-300 mt-2 leading-relaxed">
                An admin will contact you shortly regarding your campaign. Once approved,
                you'll be required to fund your wallet with <span className="text-emerald-300 font-bold">${total.toFixed(2)}</span>
                {" "}(equivalent of your ad total). Only then will the campaign go live.
              </p>
              <label className="mt-4 flex items-start gap-3 p-3 rounded-lg bg-[#0f0f12] border border-white/10 cursor-pointer">
                <input type="checkbox" checked={ack} onChange={(e) => setAck(e.target.checked)} className="mt-0.5 w-4 h-4 accent-emerald-500" />
                <span className="text-xs text-slate-300 font-semibold">I acknowledge and agree to fund my wallet before the campaign starts.</span>
              </label>
              <div className="mt-5 flex gap-2">
                <button onClick={() => setConfirmOpen(false)} className="flex-1 h-10 rounded-full bg-white/5 border border-white/10 text-white text-xs font-bold">Cancel</button>
                <button disabled={!ack || saving} onClick={doSubmit} className="flex-1 h-10 rounded-full bg-emerald-500 hover:bg-emerald-400 disabled:opacity-50 text-black text-xs font-black inline-flex items-center justify-center gap-2">
...
function Label({ children }: { children: React.ReactNode }) {
  return <div className="text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-1.5 mt-2">{children}</div>;
}
