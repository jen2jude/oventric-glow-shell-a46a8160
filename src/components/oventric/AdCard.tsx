...
export function AdCard({ ad, variant = "card" }: { ad: AdminAd; variant?: "card" | "banner" }) {
  ...
  return (
    <a
      ...
      className={`group block bg-[#1E1E24] border border-fuchsia-500/30 rounded-xl overflow-hidden hover:border-fuchsia-400/60 transition-colors ${
    ...
    >
      {ad.tier !== "text" && (
        <div className={`relative bg-gradient-to-br from-fuchsia-600 to-purple-800 ${isBanner ? "h-24" : "h-32"} overflow-hidden`}>
          {ad.mediaUrl && (
            <ResponsiveImage src={ad.mediaUrl} alt={ad.advertiser} className="absolute inset-0 w-full h-full object-cover opacity-70"  loading="lazy" decoding="async" />
          )}
          {ad.tier === "video" && (
      ...
      <div className="p-3 flex flex-col gap-2 flex-1">
        ...
        <div className="text-white font-bold text-sm leading-snug line-clamp-2">{ad.advertiser}</div>
        <div className="text-[11px] text-slate-500 line-clamp-2 flex-1">Promoted placement · {ad.tier}</div>
